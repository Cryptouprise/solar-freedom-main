...
export default function AttorneyPipeline() {
  ...
  const research = trpc.agents.runAttorneyResearch.useMutation({
    onSuccess: (result) => {
      utils.agents.listAttorneys.invalidate();
      if (result.status === "blocked") toast.info(result.reason);
      else toast.success(`Research complete: ${result.saved} verified prospects added from ${result.states.join(", ") || "selected states"}`);
    },
    onError: (error) => toast.error(error.message),
  });
